...
class MyHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
	    ...
	    l = 0
            ...
            self.wfile.write("Column not present")
            self.wfile.write("</body></html>")            
        return
...
if __name__ == '__main__':
	...
	init()
        ...
        f.write("server started at"+time.ctime()+"\n")
        f.close()
        ...
        httpd.server_close()
        f = open("serverLog.txt","a")
        f.write("server closed normally at time: "+time.ctime()+"\n")
        f.close()        
        print time.asctime(), "Server Stops - %s:%s" % (HOST_NAME, PORT_NUMBER)
    except Exception as e:
        f = open("serverLog.txt","a")
        f.write("server closed due to exception: "+str(e)+"at time: "+time.ctime()+"\n")
        f.close()        
        print e
